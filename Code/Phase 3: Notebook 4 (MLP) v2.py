...
# MAGIC %md
# MAGIC
# MAGIC ## MLP with features selected by LR and balancing of training set
...
from pyspark.ml.classification import LogisticRegression as LR, MultilayerPerceptronClassifier as MLP
from pyspark.ml.evaluation import MulticlassClassificationEvaluator as MLPEval
...
predictions_train = []
feature_maps = []
layers = [len(input_cols), len(input_cols)+10, len(input_cols)+10, 2]
for train_df, val_df in balanced_sets:
    mlp = MLP(featuresCol='feat_scaled', labelCol='label', layers = layers, seed=42, maxIter=20)
    pipe = Pipeline(stages=[assembler, scaler, lbl_indexer, mlp])
...
# MAGIC %md
# MAGIC ### Retrain on full train set
...
layers = [len(input_cols), len(input_cols)+10, len(input_cols)+10, 2]
MLP_new = MLP(featuresCol='feat_scaled', labelCol='label', layers = layers, seed=42, maxIter=20)

pipeline_new = Pipeline(stages=[assembler_new, scaler_new, lbl_indexer_new, MLP_new])
...
pred_mlp = pipeline_new_model.transform(val_data)
