...
numeric_columns = [col for col in numeric_columns if col not in ['DEP_DELAY_NEW', 'DEP_DELAY', 'DEP_DEL15', 'CARRIER_DELAY', 'ARR_DELAY', 'NAS_DELAY', 'SECURITY_DELAY', 'CANCELLED', 'WEATHER_DELAY', 'DIVERTED', 'ARR_DELAY', 'ARR_DELAY_NEW', 'ARR_DEL15']]
...
relev_data = relev_data.dropna(subset=['label'])
relev_data = relev_data.fillna(0, subset=numeric_columns)
relev_data.count()
"""
...
#overwriting above
numeric_columns = ['DISTANCE', 'origin_HourlyDryBulbTemperature', 'origin_HourlyStationPressure', 'origin_HourlyPressureChange', 'origin_HourlyWindGustSpeed', 'origin_HourlyWindDirection', 'origin_HourlyPrecipitation','origin_HourlyVisibility', 'dest_HourlyDryBulbTemperature', 'dest_HourlyStationPressure', 'dest_HourlyPressureChange', 'dest_HourlyWindGustSpeed', 'dest_HourlyWindDirection', 'dest_HourlyPrecipitation', 'dest_HourlyVisibility', 'origin_DailyPrecipitation', 'origin_DailyDepartureFromNormalAverageTemperature', 'origin_DailyAverageRelativeHumidity', 'origin_DailyAverageStationPressure', 'origin_DailySustainedWindSpeed', 'dest_DailyPrecipitation', 'dest_DailyDepartureFromNormalAverageTemperature', 'dest_DailyAverageRelativeHumidity', 'dest_DailyAverageStationPressure',  'dest_DailySustainedWindDirection', 'dest_DailySustainedWindSpeed']
...
# DBTITLE 1,Cast Values, Numeric Features (small set)
#overwriting above
numeric_columns = ['DISTANCE', 'origin_HourlyDryBulbTemperature', 'origin_HourlyStationPressure', 'origin_HourlyPressureChange', 'origin_HourlyWindGustSpeed', 'origin_HourlyWindDirection', 'origin_HourlyPrecipitation','origin_HourlyVisibility', 'dest_HourlyDryBulbTemperature', 'dest_HourlyStationPressure', 'dest_HourlyPressureChange', 'dest_HourlyWindGustSpeed', 'dest_HourlyWindDirection', 'dest_HourlyPrecipitation', 'dest_HourlyVisibility', 'origin_DailyPrecipitation', 'origin_DailyDepartureFromNormalAverageTemperature', 'origin_DailyAverageRelativeHumidity', 'origin_DailyAverageStationPressure', 'origin_DailySustainedWindSpeed', 'dest_DailyPrecipitation', 'dest_DailyDepartureFromNormalAverageTemperature', 'dest_DailyAverageRelativeHumidity', 'dest_DailyAverageStationPressure',  'dest_DailySustainedWindDirection', 'dest_DailySustainedWindSpeed']
...
relev_data = joined3M.select(*numeric_columns, col("sched_depart_date_time_UTC"), col("DEP_DEL15").alias("label"))
relev_data = relev_data.dropna(subset=['label']) 
relev_data = relev_data.fillna(0, subset=numeric_columns) ### had to impute na's because only 182 columns remained
relev_data.count()
...
# DBTITLE 1,Exp 2: numTrees=50, maxBins=20, maxDepth=10, minInstancesPerNode=2
# Define Random Forest Classifier and model
rf_2 = RandomForestClassifier(featuresCol = 'features', labelCol = 'label', numTrees=50, maxBins=20, maxDepth=10, minInstancesPerNode=2)
